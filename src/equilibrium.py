# co2_equilibrium.py
# Solve a system of equations for the concentrations of nine species for CO2 adducts in solution.
...
class SpeciesConcentration:
    ...
    def charge_neg(self) -> float:
        ...
        return 1.0 * (self.OH + self.HCO3 + self.AQH) + 2.0 * (self.AQ + self.CO3 + self.AQCO2_2)
...
def species_conc(H: float, AQH2: float, AQ_total: float, pCO2: float, pc: PhysicalConstants = pc_dflt) \
    ...
    # The amount of potassium matches the total of AQ species
...
def solve_CO2_conc(AQ_total: float, pCO2: float):
    ...
    tol: float = np.finfo(dtype=np.float64).eps*20
    options: dict = {'maxiter': 30000, 'disp': False}
    # Call the optimizer
    res: OptimizeResult = minimize(fun=fun, x0=x0, method='Nelder-Mead', tol=tol, options=options)
